    ...
    return (
      ...
      <div className="absolute left-0 top-0 h-full w-2/3 overflow-hidden">
          <div className="w-72 h-72 absolute left-0 -top-2 rounded-ee-full bg-gradient-to-r from-[#fee6b7] via-[#fff2da] to-[#fef9ec]"></div>
            ...
            <div>
            <h1 className="text-4xl font-bold relative top-8 pl-8 w-screen ">Secretive Asset</h1>
            </div>
        <div className="flex flex-wrap justify-center absolute top-24 left-12">
            ...
            {data.map((item, index) => (
                <div key={index} className="relative flex flex-col items-center p-12 mt-12 bg-white rounded shadow-2xl mx-4 my-4 border-2 border-slate-300 ">
                    <div className="w-full aspect-square">
                        Prompt: {item.prompt}
                        <div>URI: <a href={item.url} target="_blank" rel="noopener noreferrer">View</a></div>
                        <div>PRICE: <a href={item.url} target="_blank" rel="noopener noreferrer">2 Matic or $1.43</a></div>
                    </div>
